document.addEventListener("DOMContentLoaded", () => {
  ...
  const ALL_ROUTES = [
    ...BUS_ROUTES.map((r) => "А" + r),
    ...TROLLEY_ROUTES.map((r) => "T" + r),
  ];
  ...
  function createBadgeIcon(route, bearing, dir) {
    ...
    const svg = `
      <svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
        <polygon points="${c - r / 2},${c + r * 0.8} ${c + r / 2},${
      c + r * 0.8
    } ${c},${c + r + aLen}"
                 fill="${color}"
  ...
  async function updateBusMarkers(route, dir) {
    const url =
      `https://uaservice.kentkart.com/rl1/web/pathInfo?region=118&lang=uk&authType=4` +
      `&displayRouteCode=${encodeURIComponent(
        route
      )}&direction=${dir}&resultType=010000`;
    let json;
    try {
    ...
    for (const path of json.pathList || []) {
      ...
      for (const b of path.busList || []) {
        ...
        if (existing[id]) {
          ...
          existing[id].setIcon(
            createBadgeIcon(route, bearing - mapBearing, dir)
          );
  ...
  async function loadRoute(route, dir) {
    const url =
      ...
      `&displayRouteCode=${encodeURIComponent(
        route
      )}&direction=${dir}&resultType=110000`;
    let json;
    try {
      json = await fetch(url).then((r) => r.json());
    } catch (e) {
  ...
  async function loadStops(route, dir) {
    const url =
      ...
      `&displayRouteCode=${encodeURIComponent(
        route
      )}&direction=${dir}&resultType=0110000`;
    let json;
    try {
      json = await fetch(url).then((r) => r.json());
    } catch (e) {
      console.error("Fetch stops error", e);
      return;
    }
        ...
        for (const [d, layer] of Object.entries(dirs)) {
          layer.eachLayer((item) => {
            const ok =
              ...
              +selectedRoute.dir === +d;
            if (item.setOpacity)
              item.setOpacity(ok || !selectedRoute ? 1 : 0.2);
            else if (item.setStyle)
              item.setStyle({ opacity: ok || !selectedRoute ? 1 : 0.2 });
          });
        }
      }
    }
  ...
  function buildSidebar() {
    ...
    ALL_ROUTES.forEach((route) => {
      // Визначаємо тип маршруту по його імені:
      let typeClass = "";
      if (route.startsWith("А")) typeClass = "bus-btn";
      else if (route.startsWith("Т")) typeClass = "trolley-btn";
      else typeClass = "";
      
      const div = document.createElement("div");
      div.className = "route-item";
      div.innerHTML = `
        <button type="button" class="dir-btn ${typeClass}" data-route="${route}" data-dir="0">↑ ${route}</button>
        <button type="button" class="dir-btn ${typeClass}" data-route="${route}" data-dir="1">↓ ${route}</button>
      `;
      list.append(div);
    });
    ...
    list.addEventListener("click", async (e) => {
      const btn = e.target;
      if (!btn.classList.contains("dir-btn")) return;
      const route = btn.dataset.route;
      const dir = +btn.dataset.dir;
      const isActive = btn.classList.contains("is-active");
      if (!isActive) {
        // Активуємо кнопку: додаємо клас, завантажуємо дані й додаємо шари
        ...
        // Деактивуємо кнопку: видаляємо клас, ховаємо відповідні шари
        btn.classList.remove("is-active");
        if (layers.routes[route]?.[dir])
          map.removeLayer(layers.routes[route][dir]);
        if (layers.buses[route]?.[dir])
          map.removeLayer(layers.buses[route][dir]);
        if (layers.stops[route]?.[dir])
          map.removeLayer(layers.stops[route][dir]);
      }
      // Оновлення прогрес-бару та кнопки очищення
      updateProgressVisibility();
      updateClearButtonState();
      // Зберігаємо тільки активні кнопки
      settings.selected = Array.from(
        document.querySelectorAll('#routes-list .dir-btn.is-active')
      ).map((btn) => `${btn.dataset.route}_${btn.dataset.dir}`);
  ...
  document
    .querySelectorAll("#routes-list .dir-btn.is-active")
    .forEach((btn) => {
  ...
  document.getElementById("refresh-btn").addEventListener("click", () => {
    document
      .querySelectorAll('#routes-list .dir-btn.is-active')
      .forEach((btn) => updateBusMarkers(btn.dataset.route, +btn.dataset.dir));
    // перезапустити анімацію прогрес-бару
    startProgressAnimation();
  });

  // кнопка "Переключити напрями" — toggle directions for each route (NEW LOGIC)
  document.getElementById("toggle-dir-btn").addEventListener("click", () => {
    ALL_ROUTES.forEach((route) => {
      const btnUp = document.querySelector(
        `#routes-list .dir-btn[data-route="${route}"][data-dir="0"]`
      );
      const btnDown = document.querySelector(
        `#routes-list .dir-btn[data-route="${route}"][data-dir="1"]`
      );
      const upActive = btnUp && btnUp.classList.contains("is-active");
      const downActive = btnDown && btnDown.classList.contains("is-active");

      if (upActive && !downActive) {
        // переключаємо з "вгору" на "вниз"
        btnUp.classList.remove("is-active");
        if (layers.routes[route]?.[0]) map.removeLayer(layers.routes[route][0]);
        if (layers.buses[route]?.[0]) map.removeLayer(layers.buses[route][0]);
        if (layers.stops[route]?.[0]) map.removeLayer(layers.stops[route][0]);

        btnDown.classList.add("is-active");
        loadRoute(route, 1).then(() => {
          layers.routes[route][1]?.addTo(map);
          layers.buses[route][1]?.addTo(map);
          if (showStops) {
            loadStops(route, 1).then(() => {
              layers.stops[route][1]?.addTo(map);
            });
          }
        });
        ...
        if (layers.routes[route]?.[1]) map.removeLayer(layers.routes[route][1]);
        if (layers.buses[route]?.[1]) map.removeLayer(layers.buses[route][1]);
        if (layers.stops[route]?.[1]) map.removeLayer(layers.stops[route][1]);

        btnUp.classList.add("is-active");
        loadRoute(route, 0).then(() => {
          layers.routes[route][0]?.addTo(map);
          layers.buses[route][0]?.addTo(map);
          if (showStops) {
            loadStops(route, 0).then(() => {
              layers.stops[route][0]?.addTo(map);
            });
          }
        });
      }
      // Якщо обидві або жодна — не змінюємо.
    });
  ...
  // ===  "Найближчі зупинки" ===
  document
    .getElementById("nearest-stops-btn")
    .addEventListener("click", async () => {
      ...
      const stops = (json.stopList || []).filter(
        (s) => s.routes && s.routes.trim() !== ""
      );
      ...
      stops.forEach((s) => {
        ...
        marker.on("click", () => {
          // clear all active route-direction buttons
          document
            .querySelectorAll('#routes-list .dir-btn.is-active')
            .forEach((btn) => {
              btn.classList.remove('is-active');
          ...
          routeCodes.forEach((routeCode) => {
            ...
            [0, 1].forEach((d) => {
              const btn = document.querySelector(
                `#routes-list .dir-btn[data-route="${routeCode}"][data-dir="${d}"]`
              );
              if (btn && !btn.classList.contains('is-active')) {
                btn.classList.add('is-active');
                loadRoute(routeCode, d).then(() => {
                  layers.routes[routeCode][d]?.addTo(map);
                  layers.buses[routeCode][d]?.addTo(map);
                  if (showStops) {
                    loadStops(routeCode, d).then(() => {
                      layers.stops[routeCode][d]?.addTo(map);
                    });
                  }
                });
              }
  ...
  // 11.x Очистити маршрути — знімає всі активні кнопки напрямків і ховає шари маршрутів, автобусів та зупинок
  document.getElementById("clear-routes-btn").addEventListener("click", () => {
    // Знімаємо всі активні кнопки напрямків
    document
      .querySelectorAll('#routes-list .dir-btn.is-active')
      .forEach((btn) => {
        btn.classList.remove('is-active');
  ...
  settings.selected.forEach((key) => {
    ...
    const btn = document.querySelector(
      `#routes-list .dir-btn[data-route="${route}"][data-dir="${dir}"]`
    );
    if (btn && !btn.classList.contains("is-active")) {
      btn.classList.add("is-active");
      // Запускаємо код для завантаження шару
      loadRoute(route, +dir).then(() => {
        layers.routes[route][dir]?.addTo(map);
        layers.buses[route][dir]?.addTo(map);
        if (showStops) {
          loadStops(route, +dir).then(() => {
            layers.stops[route][dir]?.addTo(map);
          });
        }
      });
  ...
  if (settings.showStops) {
    ...
    stopsBtn.querySelector("span:last-child").textContent =
      "Приховувати зупинки";
  }
  ...
  function anyRouteVisible() {
    return (
      document.querySelectorAll("#routes-list .dir-btn.is-active").length > 0
    );
  }
